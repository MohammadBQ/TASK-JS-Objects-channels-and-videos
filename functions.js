...
function channelHasVideo(videoTitle, channel) {
  let  theoutcome = channel.videos.some((video) => video.title === videoTitle )
  if (theoutcome){
    return true;
  } else{
    return false;
...
function getChannelByName(channelName, channels) {
  return channels.find((vid) => {
    return vid.name === channelName
  })
}
...
function searchChannels(query, channels) {
  return channels.filter(
    (mychannel) => mychannel.name.includes(query) || mychannel.description.includes(query)
